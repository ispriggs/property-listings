    ...
    if (calState.checkOut) {
      ...
      if (listing && !listing.poa) {
        ...
        if (subtotal > 0) {
          ...
          const platformFee  = Math.round((subtotal + cleaning) * 4 / 100);
          ...
          feeRows += `<div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="color:var(--stone)">Community give back (2%)</span><span>${fmtM(communityFee)}</span></div>`;
          feeRows += `<div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="color:var(--stone)">Ecovilla Rentals platform fee (4%)</span><span>${fmtM(platformFee)}</span></div>`;
